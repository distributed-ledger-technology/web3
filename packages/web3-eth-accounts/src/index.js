...
Accounts.prototype.signTransaction = function signTransaction(tx, privateKey, callback) {
  ...
  function signed(tx) {
    ...
    try {
      const transaction = helpers.formatters.inputCallFormatter({ ...tx });
...
function _validateTransactionForSigning(tx) {
  ...
  } else if (tx.maxPriorityFeePerGas < 0 || tx.maxFeePerGas < 0) {
...
Accounts.prototype.decrypt = function (v3Keystore, password, nonStrict) {
  ...
  const seed = `0x${Buffer.from([...decipher.update(ciphertext), ...decipher.final()]).toString('hex')}`;
...
Accounts.prototype.encrypt = function (privateKey, password, options) {
  ...
  const account = this.privateKeyToAccount(privateKey, true);
...
Wallet.prototype.encrypt = function (password, options) {
  ...
  const accounts = indexes.map((index) => _this[index].encrypt(password, options));
