 ...
 */
const inputStorageKeysFormatter = function (keys) {
 ...
 * @returns {object}
 */
const outputProofFormatter = function (proof) {
...
const _txInputFormatter = function (options) {
  ...
  ['gasPrice', 'gas', 'value', 'maxPriorityFeePerGas', 'maxFeePerGas', 'nonce'].filter((key) => options[key] !== undefined).forEach((key) => {
...
const inputLogFormatter = function (options) {
  ...
  options.topics = options.topics.map((topic) => ((Array.isArray(topic)) ? topic.map(toTopic) : toTopic(topic)));

  toTopic = null;

  if (options.address) {
    options.address = (Array.isArray(options.address)) ? options.address.map((addr) => inputAddressFormatter(addr)) : inputAddressFormatter(options.address);
  }
...
const inputPostFormatter = function (post) {
  ...
  post.topics = post.topics.map((topic) => 
    ...
    ((topic.indexOf('0x') === 0) ? topic : utils.fromUtf8(topic)));
...
const outputPostFormatter = function (post) {
  ...
  post.topics = post.topics.map((topic) => utils.toUtf8(topic));
