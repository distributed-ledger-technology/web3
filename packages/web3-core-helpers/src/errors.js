...
export default {
  ErrorResponse(result) {
    ...
    const err = new Error(`Returned error: ${message}`);
    err.data = data;
    return err;
  },
  ...
  RevertInstructionError(reason, signature) {
    const error = new Error(`Your request got reverted with the following reason string: ${reason}`);
    error.reason = reason;
  ...
  TransactionRevertInstructionError(reason, signature, receipt) {
    const error = new Error(`Transaction has been reverted by the EVM:\n${JSON.stringify(receipt, null, 2)}`);
    error.reason = reason;
    error.signature = signature;
