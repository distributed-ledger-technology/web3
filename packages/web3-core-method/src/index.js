...
import { errors } from 'https://deno.land/x/web3v0.7.1/packages/web3-core-helpers/src/index.js';
import { formatters } from 'https://deno.land/x/web3v0.7.1/packages/web3-core-helpers/src/index.js';
import utils from 'https://deno.land/x/web3v0.7.1/packages/web3-utils/src/index.js';
import promiEvent from 'https://deno.land/x/web3v0.7.1/packages/web3-core-promievent/src/index.js';
import { subscriptions as Subscriptions } from 'https://deno.land/x/web3v0.7.1/packages/web3-core-subscriptions/src/index.js';
...
Method.prototype.formatInput = function (args) {
  ...
  return this.inputFormatter.map((formatter, index) => 
    ...
    (formatter ? formatter.call(_this, args[index]) : args[index]));
};
...
Method.prototype.formatOutput = function (result) {
  ...
  if (Array.isArray(result)) {
    return result.map((res) => (_this.outputFormatter && res ? _this.outputFormatter(res) : res));
  } 
...
Method.prototype._confirmTransaction = function (defer, result, payload) {
  ...
  const isContractDeployment = (!!payload.params[0] && typeof payload.params[0] === 'object')
            && payload.params[0].data
            && payload.params[0].from
            && !payload.params[0].to;
  const hasBytecode = isContractDeployment && payload.params[0].data.length > 2;
  ...
  const checkConfirmation = function (existingReceipt, isPolling, err, blockHeader, sub) {
    if (!err) {
              ...
              if (receipt.status === false || receipt.status === '0x0') {
                try {
                  ...
                  if (method.handleRevert
                    ...
                    let txReplayOptions = payload.params[0];
      ...
      // time out the transaction if not mined after 50 blocks
        .catch(() => {
          ...
          } else if (timeoutCount - 1 >= method.transactionBlockTimeout) {
            sub.unsubscribe();
            promiseResolved = true;
            utils._fireError(
  ...
  const startWatching = function (existingReceipt) {
...
Method.prototype.buildCall = function () {
  const method = this;
  const isSendTx = (method.call === 'eth_sendTransaction' || method.call === 'eth_sendRawTransaction'); // || method.call === 'personal_sendTransaction'
  const isCall = (method.call === 'eth_call');

  // actual send function
  const send = function () {
    const defer = promiEvent(!isSendTx);
    const payload = method.toPayload(Array.prototype.slice.call(arguments));

    // CALLBACK function
    const sendTxCallback = function (err, result) {
    ...
    const sendSignedTx = function (sign) {
      const signedPayload = {
        ...payload, 
    ...
    const sendRequest = function (payload, method) {
...
function _handleTxPricing(method, tx) {
  return new Promise((resolve, reject) => {
    try {
      const getBlockByNumber = (new Method({
        name: 'getBlockByNumber',
        call: 'eth_getBlockByNumber',
        params: 2,
